  ...
  <view class="">
    ...
    <view class="total_line">
      <view class="line_item">
        <view class="txt_info">
          <view>库存总类</view>
          <view><text>{{ place_stock_category }}</text>种</view>
        </view>
      ...
      <view class="line_item">
        <view class="txt_info">
          <view>库存总量</view>
          <view><text>{{ place_stock_sum }}</text>件</view>
        </view>
...
<script>
  import { httpStockInfo } from '@/utils/request/api/position.js'
  ...
  export default {
    ...
    methods: {
      ...
      toOperation (item) {
        uni.showActionSheet({
            ...
            success: function (res) {
                uni.navigateTo({
                  url: '/pages/position/putsetting?type=' + ( res.tapIndex + 1) + '&goodsId=' + item.goods_id
                })
            },
            fail: function (res) {
      ...
      async getStockInfo () {
        ...
        this.dataList = msg.place_stock_info
        this.place_stock_category = msg.place_stock_category
        this.place_stock_sum = msg.place_stock_sum
      }
    },
    onShow() {
      this.getStockInfo()
